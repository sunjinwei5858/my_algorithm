...
public class _03_longest_increasing_subsequence {
    ...
    /**
     * 非常规的动态规划
     * 状态定义定义：dp数组，dp[i]表示以num[i]这个数结尾的最长递增子序列的长度，这是解决子序列问题的一个套路,一维。
     * 2。状态转移方程：数学归纳法 假设[0,..,i-1]已经算出来了，那么dp[i]:看num[i]和num[i-1]的大小，
     * 如果num[i]比num[i-1]更大，那么dp[i]=max(dp[i],dp[i-1]+1)
    ...
    public int lengthOfLIS02(int[] nums) {
        // 状态定义：dptable 一维 ，dp[i]的值代表nums前i个数字的最长子序列长度
        int[] dpTable = new int[nums.length];
